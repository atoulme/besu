...
{
    proxy = await
    TestPermissioning.new();
    let permitted = await
    proxy.enodeAllowed(node1High, node1Low, node1Host, node1Port);
...
let key2 = await
proxy.computeKey(node1High, node1Low, node1Host, node1Port);
assert.equal(key1, key2, "computed keys should be the same");

let key3 = await
proxy.computeKey(node1High, node1Low, node1Host, node2Port);
...
// await another
await
proxy.addEnode(node2High, node2Low, node2Host, node2Port);
permitted = await
proxy.enodeAllowed(node2High, node2Low, node2Host, node2Port);
...
it('Should allow a connection between 2 added nodes', async() = > {
    let permitted = await
    proxy
    .connectionAllowed(node1High, node1Low, node1Host, node1Port, node2High, node2Low, node2Host, node2Port);
...
permitted = await
proxy.connectionAllowed(node1High, node1Low, node1Host, node1Port, node2High, node2Low, node2Host, node2Port);
